...
// impl<W: WeightToFeeConverter, R: TakeRevenue> WeightTrader for
// AssetRegistryTrader<W, R> { 	fn new() -> Self {
// 		Self {
// 			bought_weight: None,
...
// 		if matches!(self.bought_weight, Some(ref bought) if &bought.asset_location
// != location) { 			// we already bought another asset - don't attempt to buy
// this one since 			// we won't be able to refund it
// 			continue;
// 		}
...
// 			if let Ok(unused) = payment.clone().checked_sub((asset.clone(),
// fee_increase).into()) { 				let (existing_weight, existing_fee) = match
// self.bought_weight { 					Some(ref x) => (x.weight, x.amount),
// 					None => (Weight::zero(), 0),
// 				};

// 				self.bought_weight = Some(BoughtWeight {
// 					amount:
// existing_fee.checked_add(fee_increase).ok_or(XcmError::Overflow)?,
...
// 	fn refund_weight(&mut self, weight: XcmWeight, _context: &XcmContext) ->
// Option<Asset> { 		log::trace!(target: "xcm::weight",
// "AssetRegistryTrader::refund_weight weight: {:?}", weight);

// 		match self.bought_weight {
// 			Some(ref mut bought) => {
// 				let new_weight = bought.weight.saturating_sub(weight);
// 				let new_amount = W::convert_weight_to_fee(&bought.asset_location,
// new_weight)?; 				let refunded_amount =
...
// impl<W: WeightToFeeConverter, R: TakeRevenue> Drop for AssetRegistryTrader<W,
// R> { 	fn drop(&mut self) {
// 		if let Some(ref bought) = self.bought_weight {
// 			R::take_revenue((AssetId(bought.asset_location.clone()),
// bought.amount).into()); 		}
// 	}
// }
...
impl<T: Config> Inspect for Pallet<T> {
	...
	// 	Pallet::<T>::location_to_asset_id(v3::Location::try_from(location.clone()).
	// ok()?) }
	...
	// Self::StringLimit>> { 	Pallet::<T>::fetch_metadata_by_location(&
	// v3::Location::try_from(location.clone()).ok()?) }

	// fn location(asset_id: &Self::AssetId) -> Result<Option<Location>,
	// DispatchError> { 	Pallet::<T>::location(asset_id).map(|l| l.and_then(|l|
	// l.try_into().ok())) }
}
