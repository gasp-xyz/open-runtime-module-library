...
pub mod module {
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			...
			assert!(
				unique_endowed_accounts.len() == self.tokens_endowment.len(),
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn transfer(
			...
			Self::do_transfer(currency_id.into(), &from, &to, amount, ExistenceRequirement::AllowDeath)?;

			Self::deposit_event(Event::Transfer(currency_id.into(), from, to, amount));
			Ok(())
		}
	...
	T: Config,
{
	...
	fn deposit_into_existing(
		...
		Pallet::<T>::do_deposit(currency_id, who, value, true, false).map(|_| MultiTokenPositiveImbalance::new(currency_id, value))
	}
	...
	fn deposit_creating(
		...
		Pallet::<T>::do_deposit(currency_id, who, value, false, false)
			.map_or_else(|_| Self::PositiveImbalance::zero(currency_id), |_| MultiTokenPositiveImbalance::new(currency_id, value))
	}
